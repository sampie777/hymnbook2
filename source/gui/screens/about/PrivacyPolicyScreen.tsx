...
const PrivacyPolicyScreen: React.FC = () => {
  ...
  return (<ScrollView style={styles.container}>
    <View style={styles.content}>
      ...
      <Text style={[styles.contentText]}>
        BACKEND. This app or it's backend doesn't collect any personal information from you. The authentication with the backend makes use of an identifier that is unique to your device. Don't worry, no one can link this identifier to your device, as we hash the device identifier before it leaves your device. The hash is irreversible, so the backend receives gibberish and this gibberish makes you completely anonymous. The important thing is that this gibberish remains the same for each device, even after app re-installs. That way the authorization with the backend for each device can made be persistent. The use of the hashed device ID eliminates the use of user accounts.
      </Text>
