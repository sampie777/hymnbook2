...
describe("Song search find songs by title", () => {
  ...
  it("find main title (1)", () => {
    const result = SongSearch.findByTitle("Psalm");
    expect(result.length).toBe(3);
    expect(result[0].name).toBe("Psalm 1");
    expect(result[1].name).toBe("Psalm 2");
    expect(result[2].name).toBe("Psalm 3");
  });

  it("find main title (2)", () => {
    const result = SongSearch.findByTitle("Psalm 2");
    expect(result.length).toBe(1);
    expect(result[0].name).toBe("Psalm 2");
  });

  it("find nothing if main title does not exist", () => {
    const result = SongSearch.findByTitle("XXX");
    expect(result.length).toBe(0);
  });

  it("find by alternative titles", () => {
    const result = SongSearch.findByTitle("Lofsang");
  ...
  it("find by alternative and main titles", () => {
    const result = SongSearch.findByTitle("Sagaria");
    expect(result.length).toBe(2);
    expect(result[0].name).toBe("Psalm 2");
    expect(result[1].name).toBe("Sagaria");
  });
});
